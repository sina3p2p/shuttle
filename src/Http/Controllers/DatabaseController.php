...
class DatabaseController extends Controller
{
    public function index()
    {
        ...
        return view('shuttle::database.index', compact('tables'));
    }

    public function create()
    {
        $db = $this->prepareDbManager('create');
        return view('shuttle::database.edit_add', compact('db'));
    }
...
//            }
            ...
            return redirect()->route('shuttle.database.index');
//        } catch (Exception $e) {
//            dd($e);
...
//            DatabaseUpdater::update($table);
    ...
    public function show($table)
    {
        ...
        $model_name = ScaffoldInterface::where('name', $table)->pluck('model')->first();
        if (isset($model_name)) {
...
//        return response()->json(collect(SchemaManager::describeTable(app($request->model)->getTable())));
        $additional_attributes = [];
        $model_name = optional(ScaffoldInterface::where('name', $request->model)->first())->translation_model;
        if (isset($model_name)) {
            $model = app($model_name);
...
//            }
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('shuttle.database.update', $table);
        } else {
            ...
            $db->formAction = route('shuttle.database.store');
        }
