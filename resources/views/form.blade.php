<form action="{{ $edit ? route('shuttle.scaffold_interface.update', ['scaffold_interface' => $scaffoldInterface, 'id' => $dataTypeContent->id, 'lang' => $lang]) : route('shuttle.scaffold_interface.store',$scaffoldInterface) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @if($edit)
        @method('PUT')
    @endif
