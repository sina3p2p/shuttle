...
$.dore = function(element, options) {
  ...
  function init() {
    ...
    function getActiveMainMenuLink() {
      ...
      pageUrl = myurl.replace(regex,'') + 'mygo';
      if(current[1]) {
        pageUrl += '/' + current[1]
      }
      ...
      selector.each(function () {
        ...
        $('.main-menu > div.scroll').animate({
          scrollTop: parseInt( $(this).parents('li').offset().top) - $(this).parents('li').height()
        }, 50);
        let p = $(this).parents('ul.list-unstyled[data-link]');
        if(p){
          $("a[href='#"+p.data("link")+"']").parents('li').addClass('active');
        }
      });
    ...
    if ($().dropzone && !drop.hasClass("disabled")) {
      drop.dropzone({
        ...
        success: function (file, response) {
          let el = '<div class="col-sm-12 col-md-6 col-xl-4 sfl2-multiple-item">\n' +
              ...
              '<img src="'+response.url+'" alt="uploaded image" class="list-media-thumbnail responsive border-0 sfl-selected-item-image" />' +
              '<input name="'+drop.data('name')+'[]" class="sfl-input-item" value="'+response.path+'" hidden></a>' +
              ...
              '<a class="w-100"><p class="list-item-heading mb-1 truncate sfl-selected-item-label">'+response.name+'</p></a></div>' +
    ...
    if ($().selectFromLibrary) {
      if($("#libraryModal").length === 0) {
        $('body').append('<div class="modal fade modal-right select-from-library" id="libraryModal" tabindex="-1" role="dialog" aria-labelledby="libraryModal" aria-hidden="true" data-backdrop="static">\n' +
            ...
            '<form action="/mygo/media/upload" class="dropzone dropzone-area disabled" id="dpz-multiple-files">' +
