...
$.sinaUploader = function(element, options) {
    ...
    if (multiple) {
        $newEl.find("#sortable" ).sortable({
            ...
            placeholder: "fileuploader-item file-main-1 fileuploader-sorter-placeholder"
        });
    }
    ...
    if($self.data('value')){
        ...
        if(multiple)
        {
            value.split(',').forEach(function (e) {
                var previewItem = $(uploaded_file_card);
                $newEl.find('ul.fileuploader-items-list').append(previewItem);
                syncElement(previewItem, {url: e.replace('public','/storage'), path: e});
            })
        }else{
            var previewItem = $(uploaded_file_card);
            $newEl.find('li.fileuploader-input').hide();
            $newEl.find('ul.fileuploader-items-list').append(previewItem);
            syncElement(previewItem, {url: value.replace('public','/storage'), path: value});
        }
    }
    ...
    function syncElement($el, config) {
        ...
        $el.find('.fileuploader-action-remove').addClass("fileuploader-action-success").removeClass("fileuploader-action-remove");
    ...
    var $modal = plugin.settings.libraryId.indexOf("#") > -1 ? $(plugin.settings.libraryId) : $("#" + plugin.settings.libraryId);
    var $itemContaines;
    var $submitButton;
