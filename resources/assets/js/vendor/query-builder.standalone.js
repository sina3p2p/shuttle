...
QueryBuilder.selectors = {
    ...
    type_container:       '.rule-type-container',
    ...
    rule_type:            '.rule-type-container [name$=_type]',
...
QueryBuilder.prototype.bindEvents = function() {
    ...
    // rule type change
    this.$el.on('change.queryBuilder', Selectors.rule_type, function() {
        var $rule = $(this).closest(Selectors.rule_container);
        self.getModel($rule).type = $(this).val();
    });
        ...
        'update': function(e, node, field, value, oldValue) {
            if (node instanceof Rule) {
                switch (field) {
                    ...
                    case 'type':
                        self.updateRuleType(node, oldValue);
                        break;
...
QueryBuilder.prototype.createRuleOperators = function(rule) {
    ...
    var $typeContainer = rule.$el.find(QueryBuilder.selectors.type_container).empty();
    ...
    var $typeSelect = $(this.getRuleTypeSelect(rule));
    $typeContainer.html($typeSelect);
    ...
    rule.__.type = QueryBuilder.DEFAULTS.default_type;

    rule.$el.find(QueryBuilder.selectors.rule_operator).val(rule.operator.type);

    rule.$el.find(QueryBuilder.selectors.rule_type).val(rule.type);
...
QueryBuilder.prototype.updateRuleType = function(rule, previousOperator) {
    rule.$el.find(QueryBuilder.selectors.rule_type).val(rule.type);
    this.trigger('afterUpdateRuleOperator', rule, previousOperator);
    this.trigger('rulesChanged');
};
    ...
    var out = (function parse(group) {
        ...
        group.each(function(rule) {
            ...
            var ruleData = {
                ...
                valueType: rule.type ? rule.type : null,
                value: value
            };
...
<div id="{{= it.group_id }}" class="rules-group-container"> \
  <div class="rules-group-header form-inline"> \
    ...
    <div class="btn-group group-conditions ml-2"> \
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  ...
  <div class="rule-type-container"></div>\
...
QueryBuilder.templates.valueTypeSelect = '\
<select class="form-control" name="{{= it.rule.id }}_type"> \
...
QueryBuilder.prototype.getRuleTypeSelect = function(rule) {
    var h = this.templates.valueTypeSelect({
        builder: this,
        rule: rule,
        icons: this.icons,
        settings: this.settings,
    ...
    return this.change('getRuleTypeSelect', h, rule);
};
...
Utils.defineModelProperties(Rule, ['filter', 'operator', 'type', 'value']);
